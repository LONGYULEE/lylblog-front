...
Toc.prototype._createTocContent = function() {
    ...
    for (var i = 0; i < this._elTitleElementsLen; i++) {
        ...
        var lastIndex = this._elTitleElementText.lastIndexOf('>');
        var finalStr = this._elTitleElementText.substring(
            lastIndex + 1,
            this._elTitleElementText.length
        );

        this.tocContent +=
            '<li><a class="toc-link tip' + i + '">' + finalStr + '</a>';
        ...
        if (j !== this._elTitleElementsLen) {
            ...
            if (this._elTitleElementName !== this._elNextTitleElementName) {
                ...
                for (var t = this._tempLists.length - 1; t >= 0; t--) {
                    if (
                        this._tempLists[t].tagName ===
                        this._elNextTitleElementName
                    ) {
                        checkColse = true;
                        break;
                    }
            ...
            if (this._tempLists.length) {
                this.tocContent += new Array(this._tempLists.length + 1).join(
                    '</li></ul>'
                );
            } else {
                this.tocContent += '</li>';
    ...
    this.tocContent =
        '<div class="active-indicator"></div><ul class="menu-root">' +
        this.tocContent +
        '</ul>';
};
